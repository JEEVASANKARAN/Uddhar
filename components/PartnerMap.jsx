...
export default function PartnerMap({ userLat, userLng, loanCategory, t }) {
  ...
  useEffect(() => {
    ...
    // Prevent double-init if Leaflet already attached
  ...
  // Update markers whenever relevant data changes, but only after map is ready
  useEffect(() => {
    if (typeof window === 'undefined') return;
    ...
    // Safely update map center
    try {
      map.setView([userLat, userLng], 11, { animate: false });
    } catch (e) {
    ...
    const createCustomIcon = (type) => {
      ...
      return L.divIcon({
        ...
        html: `<div style="
          background:${c.bg};color:#0C0F1E;
          width:${c.size}px;height:${c.size}px;border-radius:50%;
          display:flex;align-items:center;justify-content:center;
          font-weight:bold;font-size:${type === 'recommended' ? '14px' : '11px'};
          border:${c.border};box-shadow:${c.shadow};
    ...
    try {
      ...
      userMarker.bindPopup(
        `<b>📍 Applicant Location</b><br/>Lat: ${userLat.toFixed(4)}, Lng: ${userLng.toFixed(4)}`,
        { autoPan: false }
      );
      markersRef.current.push(userMarker);
    } catch (e) {
      console.warn('User marker failed:', e.message);
    }

    // Eligible partner markers
    let topMarker = null;
      ...
      try {
        ...
        marker.bindPopup(`
          <div style="font-family:IBM Plex Sans,sans-serif;padding:4px;min-width:180px;">
            ...
            <div style="margin-top:4px;font-size:10px;color:#9A9284;">📞 ${partner.phone}</div>
            <span style="display:inline-block;margin-top:6px;color:#4E9C7D;font-weight:bold;font-size:10px;text-transform:uppercase;">✓ Route Authorized</span>
          </div>
        `, { autoPan: false });
        ...
        if (isTop) topMarker = marker;
      } catch (e) {
        console.warn('Eligible marker failed:', e.message);
      }
    });

    // Excluded partner markers
    if (!filterUnhealthy) {
        ...
        try {
          ...
          marker.bindPopup(`
            <div style="font-family:IBM Plex Sans,sans-serif;padding:4px;">
              ...
              <span style="display:inline-block;margin-top:6px;color:#C77066;font-weight:bold;font-size:10px;text-transform:uppercase;">✕ Filtered Out (High Risk)</span>
            </div>
          `, { autoPan: false });
          ...
          console.warn('Excluded marker failed:', e.message);
        }
      });
    }

    // Open top popup after map is fully settled — with extra safety guard
    if (topMarker) {
      setTimeout(() => {
        try {
          if (topMarker && topMarker._map && topMarker._map._loaded && topMarker.getLatLng()) {
            topMarker.openPopup();
          }
        } catch (e) {
          console.warn('Popup open skipped:', e.message);
        }
      }, 600);
    }
  ...
  }, [userLat, userLng, loanCategory, filterUnhealthy]); // stable deps — not the array refs
  ...
  return (
    <div id="partner-map-section" style={{ marginTop: '24px' }}>
      ...
      <div id="partner-list-container">
        ...
        {eligible.length === 0 && (
          <div style={{ color: 'var(--stone)', fontSize: '13px', padding: '12px', background: 'var(--bg-deep)', borderRadius: '4px', border: '1px solid var(--rule)' }}>
            No eligible partners found for this loan category and filter. Try unchecking "Hide Unhealthy Partners" to see all options.
          </div>
        )}
        ...
        {!filterUnhealthy && excluded.length > 0 && (
          <div style={{ marginTop: '20px', paddingTop: '16px', borderTop: '1px dashed var(--rule)' }}>
            ...
            {excluded.map((partner) => (
              <div
                key={partner.id}
                id={`partner-card-excluded-${partner.id}`}
                className="partner"
                style={{ opacity: 0.65 }}
              >
                <div>
                ...
                <span id={`health-pill-excluded-${partner.id}`} className="health-pill health-bad">
